"""
FlyDSL Flash Attention Library Kernel with MFMA bf16.

Production kernel: scf.for_ runtime KV loop + MFMA Q@K^T dot product
using rocdl.mfma_f32_16x16x16bf16_1k with vector<4xi16> operands.

Each thread computes one output (row, d_pair) element.
Q@K^T dot uses vectorized dwordx4 loads + MFMA 16x16x16 accumulation.
Online softmax with exp2. P@V uses scalar accumulation.

Usage: cd /opt/FlyDSL && python /workspace/fa4_rocm/flydsl_kernels/flash_fwd_mfma_lib.py
"""
...
def build_flash_attn_module(seqlen, num_heads, batch, softmax_scale):
    """Build and compile a FlyDSL MFMA flash attention kernel.

    Returns an executor with .launch(o, q, k, v, stream_ptr) method.
    """
    arch = get_hip_arch()
    _sq = seqlen
    _nh = num_heads
    ...
    _hd2 = HD // 2
    _total = batch * num_heads * seqlen * _hd2
    n_blocks = (_total + BLOCK_SIZE - 1) // BLOCK_SIZE

    module_name = f"flash_mfma_b{batch}s{seqlen}h{num_heads}"
    ...
    class _FA(flir.MlirModule):
        ...
        def flash_kernel(
            ...
            # (batch, seqlen, heads, hdim) layout
            ...
            sk = arith.unwrap(arith.constant(_sq, index=True))

            for iv, (i_max, i_sum, i_lo, i_hi), (r_max, r_sum, r_lo, r_hi) in scf.for_(
                c0, sk, c1,
                ...
                # Q@K^T dot product with vectorized dwordx4 loads (8 bf16 per load)
                dot = zero_f
                ...
                # Online softmax
                new_max = arith.maximum(i_max, dot)
                rescale = flydsl_math.exp2(arith.unwrap((i_max - new_max) * log2e_f))
                p = flydsl_math.exp2(arith.unwrap((dot - new_max) * log2e_f))

                new_sum = i_sum * rescale + p
                new_lo = i_lo * rescale
                new_hi = i_hi * rescale

                # V[iv, 2*d_pair : 2*d_pair+2]
                v_dw = arith.index_cast(T.i32,
                    (batch_idx * stride_b + iv * stride_s + head_idx * stride_h) // c_2 + d_pair)
                ...
                scf.YieldOp([arith.unwrap(new_max), arith.unwrap(new_sum),
                              arith.unwrap(new_lo), arith.unwrap(new_hi)])
            ...
            o_dw = arith.index_cast(T.i32,
                (batch_idx * stride_b + row_idx * stride_s + head_idx * stride_h) // c_2 + d_pair)
...
def main():
    ...
    shapes = [
        (1, 128, 1, "b1s128h1"),
        (2, 2048, 32, "b2s2048h32"),
    ]

    print("=" * 70)
    print("FlyDSL MFMA Flash Attention Library Kernel")
    print("=" * 70)
    ...
    for batch, seqlen, nh, label in shapes:
        ...
        exe = build_flash_attn_module(seqlen, nh, batch, scale)
        ct = time.perf_counter() - t0
        print(f"  Compile: {ct:.1f}s")
        ...
        print(f"  Correct: {correct} (err={err:.6f})")
        ...
        results.append({
            "shape": label, "correct": correct, "error": err,
            "tflops": round(tf, 4), "latency_us": round(lat, 0),
    ...
    for r in results:
        if r["correct"]:
            print(f"  {r['shape']}: {r['tflops']:.4f} TF, {r['latency_us']:.0f} us, compile {r['compile_s']:.1f}s")
        else:
            print(f"  {r['shape']}: INCORRECT (err={r['error']:.6f})")

    with open("/workspace/fa4_rocm/flydsl_kernels/mfma_lib_results.json", "w") as f:
